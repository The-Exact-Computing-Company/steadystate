...
async fn launch_upterm_in_noenv(flake_path: &Path, working_dir: &Path) -> Result<(u32, String)> {
    let inner = "upterm host --force-command=bash";
    let nix_cmd = format!(
        "cd {} && nix develop {}#default --command {}",
        shell_escape::escape(working_dir.to_string_lossy().into()),
        shell_escape::escape(flake_path.to_string_lossy().into()),
        inner,
    );

    let mut cmd = nix_shell_command(&nix_cmd);
    cmd.stdout(std::process::Stdio::piped());
    cmd.stderr(std::process::Stdio::piped());

    tracing::info!("Starting upterm via nix develop: {}", nix_cmd);
...
async fn kill_pid(pid: u32) -> Result<()> {
    ...
    let status = Command::new("sh").arg("-c").arg(cmd).status().await.context("Failed to spawn kill")?;
    if !status.success() {
...
impl ComputeProvider for LocalComputeProvider {
    ...
    async fn start_session(&self, session: &mut Session, request: &SessionRequest) -> Result<()> {
        ...
        let local_session = LocalSession { pid, workspace_root };
