...
use crate::compute::{
    common::{
        ...
        sshd::{SshdConfig, SshdLogLevel},
        tproject,
    },
    traits::{ComputeProvider, ProviderCapabilities, RemoteExecutor, SessionHealth},
...
impl HetznerComputeProvider {
    ...
    async fn remote_preamble(
        ...
        ex.mkdir_p(PathBuf::from(&repo_path).as_path(), 0o700)
            .await?;
        ...
        let clone_out = ex
            ...
            .await?;
        if !clone_out.exit_status.success()
            && !ex
                .exists(PathBuf::from(&repo_path).join(".git").as_path())
                .await?
        {
        ...
        if let Some(auth) =
            crate::compute::common::provider_config::extract_forge_config(request).as_ref()
        {
    ...
    async fn remote_setup(
        ...
        match Self::resolve_mode(request.mode.as_deref())? {
            "collab" => {
                self.remote_setup_collab(ex, public_ip, session_id, request)
                    .await
            }
            _ => {
                self.remote_setup_pair(ex, public_ip, session_id, request)
                    .await
            }
        }
    }
    ...
    async fn remote_setup_collab(
        ...
        git.clone(&repo_path, &PathBuf::from(&canonical), None, None)
            .await?;
        ...
        git.checkout_new_branch(&PathBuf::from(&canonical), &branch_name)
            .await?;
        ...
        ex.write_file(
            PathBuf::from(format!("{}/sync-log", root)).as_path(),
            &[],
            0o666,
        )
        .await?;
        ex.write_file(
            PathBuf::from(format!("{}/activity-log", root)).as_path(),
            &[],
            0o666,
        )
        .await?;
        ...
        ex.write_file(
            PathBuf::from(format!("{}/bin/steadystate-wrapper", root)).as_path(),
            wrapper.as_bytes(),
            0o755,
        )
        .await?;
        ...
        let (port, host_pub) = self
            .launch_remote_sshd(ex, &root, &authorized_keys, &forced_command)
            .await?;
    ...
    async fn remote_setup_pair(
        ...
        ex.write_file(
            PathBuf::from(format!("{}/bin/pair-wrapper", root)).as_path(),
            wrapper.as_bytes(),
            0o755,
        )
        .await?;
    ...
    async fn launch_remote_sshd(
        ...
        if !keygen.exit_status.success() {
            return Err(anyhow!(
                "remote host key generation failed: {}",
                keygen.stderr
            ));
        }
        ...
        let content = self
            .keys
            .generate_authorized_keys_file(authorized_keys, Some(forced_command));
        ex.write_file(
            PathBuf::from(&auth_keys_path).as_path(),
            content.as_bytes(),
            0o600,
        )
        .await?;

        // Deterministic high port from root hash, with bind retries: two
        // sessions sharing an id prefix hash to the same port (1/8000 per
        // pair). Up to 8 attempts before giving up.
        let mut port: u16 = 0;
        for attempt in 0..8u64 {
            ...
            ex.write_file(
                PathBuf::from(&cfg_path).as_path(),
                cfg.generate().as_bytes(),
                0o600,
            )
            .await?;
            ...
            let _pid: u32 = match launch.stdout.trim().parse() {
                Ok(pid) => pid,
                Err(_) => {
                    // sshd likely failed to bind (port collision); retry.
                    tracing::warn!(
                        "Remote sshd launch produced no pid on port {}; retrying",
                        candidate
                    );
                    continue;
                }
            };
            // Verify the port is actually listening before declaring victory.
            let probe = ex
                .exec_shell(&format!(
                    "(exec 3<>/dev/tcp/127.0.0.1/{}) 2>/dev/null && echo open || echo closed",
                    candidate
                ))
                .await;
            match probe {
                Ok(o) if o.stdout.contains("open") => {
                    port = candidate;
                    break;
                }
                _ => {
        ...
        if port == 0 {
            return Err(anyhow!(
                "sshd failed to bind after 8 port attempts; check sshd.log on the host"
            ));
        }
        let pubkey = ex
            .read_file(PathBuf::from(format!("{}.pub", host_key)).as_path())
            .await?;
    ...
    async fn health_check(&self, session: &Session) -> Result<SessionHealth> {
        if let Some(rs) = self.sessions.get(&session.id) {
            match self.api.get_server(rs.server_id).await {
                ...
                Ok(s) => Ok(SessionHealth::Unhealthy {
                    reason: format!(
                        "server {} ({}:{}) status: {}",
                        rs.server_id, rs.ip, rs.session_sshd_port, s.status
                    ),
                }),
                Err(e) => Ok(SessionHealth::Degraded {
                    reason: format!("hcloud api for server {}: {:#}", rs.server_id, e),
                }),
            }
        } else {
...
mod tests {
    ...
    fn test_resolve_mode_matches_local() {
        ...
        assert_eq!(
            HetznerComputeProvider::resolve_mode(Some("collab")).unwrap(),
            "collab"
        );
        assert_eq!(
            HetznerComputeProvider::resolve_mode(Some("pair")).unwrap(),
            "pair"
        );
